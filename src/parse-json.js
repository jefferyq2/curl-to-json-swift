...
// Must use whatwg-url-without-unicode instead of whatwg-url because it doesn't work in JavascriptCore
const WHATWGURL = require('whatwg-url-without-unicode');
...
module.exports = exports.default = function (data) {
  ...
  if (result.url) {
    let url = new WHATWGURL.URL(result.url);
    result.url = url.origin + url.pathname;
    let params = new WHATWGURL.URLSearchParams(url.search);
